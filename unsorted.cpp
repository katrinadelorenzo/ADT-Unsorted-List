...
UnsortedType::UnsortedType(int size1)
{
  size = size1;
  info = new ItemType[size];
  length = 0;
...
ItemType &UnsortedType::operator[] (int index) // Overloading index operatgor
{
  return (info[index]);
}
...
// Post: item is in the list.
{
  if (length == size)
    {
      ItemType * new_array; // allocation of new array
      new_array = new ItemType[size + 10]; // creation of a bigger array
      for (int i = 0; i < length; i++)
	{
	  new_array[i] = info[i]; // copy first array to second array
	}
